...
\begin{center}
	...
	\normalsize{\today}\\
\end{center}
\section*{Localization of \( C[x] \)}

Let \( C[x] \) be the ring of polynomials with complex coefficients. We consider three types of localization:
\begin{itemize}
	\item Localization at a prime ideal \( (x - a) \), where \( a \in C \).
	\item Localization at the zero ideal \( (0) \).
	\item Localization at a specific polynomial \( f \in C[x] \).
\end{itemize}

\subsection*{1. Localization at \( (x - a) \)}

For a point \( a \in C \), the prime ideal \( (x - a) \) in \( C[x] \) allows localization to the ring \( C[x]_{(x - a)} \). Elements in this ring are of the form:
\[
\frac{g(x)}{h(x)} \quad \text{where } g(x), h(x) \in C[x] \text{ and } h(a) \neq 0.
\]

\subsection*{2. Localization at the Zero Ideal \( (0) \)}

Localization at the zero ideal \( (0) \) corresponds to the field of fractions \( C(x) \), consisting of all rational functions:
\[
\frac{g(x)}{h(x)} \quad \text{where } g(x), h(x) \in C[x] \text{ and } h(x) \neq 0.
\]

\subsection*{3. Localization at a Polynomial \( f \)}

For \( f \in C[x] \), the localization \( C[x]_f \) consists of elements:
\[
\frac{g(x)}{f^n} \quad \text{where } g(x) \in C[x] \text{ and } n \in \mathbb{N}.
\]

\subsection*{Graphical Representation in \( C \) (Complex Plane)}
...
\begin{center}
	\begin{tikzpicture}
		...
		% Point for prime ideal (x - a)
		\filldraw[blue] (1.5,1) circle (2pt) node[above] {$a$};
		\node[blue] at (1.5, 0.5) {$(x - a)$};
		
		% Circle around the origin for (0) ideal
		\draw[red, dashed] (0,0) circle (2.5);
		\node[red] at (1.7,2) {$(0)$};
		
		% Localization at a polynomial f(x)
		\node[green] at (-2.5,-2) {$f(x) \neq 0$};
	\end{tikzpicture}
\end{center}

In the diagram:
\begin{itemize}
	\item The blue point represents the localization at \( (x - a) \), where we focus on values near \( x = a \).
	\item The red dashed circle represents localization at the zero ideal \( (0) \), which includes all points in \( C \).
	\item The green label represents areas where \( f(x) \neq 0 \) for localization at a polynomial \( f \).
\end{itemize}
\newpage

\section*{Localization in \( C[x] \)}

Let \( C[x] \) denote the commutative ring of complex polynomials. The localization at a point \( f \), a prime ideal \( (x-a) \), and the zero ideal \( (0) \) reflects function spaces within \( C[x] \).

\begin{center}
	\begin{tikzpicture}[
		scale=1.2,
		function_space/.style={circle, draw, minimum size=4cm},
		localization_point/.style={circle, fill=blue, inner sep=1.5pt},
		localization_prime/.style={circle, fill=red, inner sep=1.5pt},
		localization_zero/.style={circle, draw=red, densely dashed, minimum size=3.5cm}
		]
		
		% Function space C[x]
		\node[function_space] (C) at (0,0) {$C[x]$};
		
		% Localization at (x - a)
		\node[localization_prime, label=above:{\small $x=a$}] (a) at (1.5,0.75) {};
		\node at (2,1.25) {\textcolor{red}{$(x - a)$}};
		\draw[-{Stealth[scale=1]}, red, thick] (C) to[out=30,in=180] (a);
		
		% Localization at the zero ideal (0)
		\node[localization_zero, label={[text=red, yshift=-0.75cm]below:{\small Zero Ideal $(0)$}}] (zero) at (0,0) {};
		\draw[-{Stealth[scale=1]}, red, thick, densely dashed] (C) to[out=-45,in=135] (1.5,-1);
		
		% Localization at a general point f
		\node[localization_point, label=below:{\small $f \neq 0$}] (f) at (-1.5,0.75) {};
		\node at (-2.5,1.5) {\textcolor{blue}{$C[x]_f$}};
		\draw[-{Stealth[scale=1]}, blue, thick] (C) to[out=150,in=0] (f);
		
		% Arrow descriptions
		\node at (0.25,-2) {Localization Map};
		\node[align=center] at (1.75,-2.2) {\textcolor{red}{At Prime Ideal}\\ \textcolor{red}{$(x - a)$}};
		\node[align=center] at (-1.5,-2.2) {\textcolor{blue}{At Point}\\ \textcolor{blue}{$f$}};
		\node at (2.5, 0.5) [align=center] {\textcolor{red}{Ring} \\ \textcolor{red}{Focus}};
		\node at (-2, -0.5) [align=center] {\textcolor{blue}{Function}\\ \textcolor{blue}{Space}};
...
In this diagram:
\begin{itemize}
	\item The circle labeled \( C[x] \) represents the function space of complex polynomials.
	\item The red dashed circle surrounding the origin corresponds to the zero ideal \( (0) \), where we obtain the field of fractions \( C(x) \).
	\item The red dot \( x = a \) represents localization at the prime ideal \( (x - a) \).
	\item The blue dot represents localization at a point \( f \) (where \( f \neq 0 \)), showing a ring where \( f \) becomes invertible.
\end{itemize}

This setup represents the localization as focusing on specific points or subspaces within the function space \( C[x] \), reflecting the commutative nature of polynomial functions over complex values.
\newpage
\section*{Illustration of Localizations in \( C[x] \)}

\begin{center}
	\begin{tikzpicture}[
		scale=1.1,
		complex_plane/.style={circle, draw, minimum size=5cm, label={above:\small Complex Plane \( C \)}},
		localization_ring/.style={circle, draw, densely dashed, minimum size=1.5cm},
		localization_near/.style={circle, fill=red, inner sep=1.5pt},
		localization_function/.style={circle, fill=blue, inner sep=1.5pt},
		localization_zero/.style={circle, draw=red, densely dashed, minimum size=4.5cm}
		]
		
		% Complex plane C
		\node[complex_plane] (C) at (0,0) {};
		
		% Localization at the prime ideal (x - a)
		\node[localization_near, label=below:{\small $a$}] (a) at (1,1) {};
		\node[localization_ring, red, label=above right:{\small Localization at \( (x - a) \)}] at (1,1) {};
		
		% Localization at the zero ideal (0)
		\node[localization_zero, label=below:{\small Localization at Zero Ideal \( (0) \)}] at (0,0) {};
		
		% Localization at a function f
		\node[localization_function, label=below:{\small $f \neq 0$}] (f) at (-1.5,1) {};
		\node[localization_ring, blue, label=above left:{\small Localization at \( f \)}] at (-1.5,1) {};
		
		% Labels
		\node[align=center] at (3.5, 1.5) {\textcolor{red}{Prime Ideal}\\ \textcolor{red}{$(x - a)$}};
		\node[align=center] at (-3.5, 1.5) {\textcolor{blue}{Invertible Function}\\ \textcolor{blue}{$f \neq 0$}};
		\node[align=center] at (0, -2.8) {\textcolor{red}{Field of Fractions}\\ \textcolor{red}{$C(x)$}};
		
		% Arrow for localization intuition
		\draw[-{Stealth[scale=1]}, red, thick] (C) to[out=45,in=210] (1,1);
		\draw[-{Stealth[scale=1]}, blue, thick] (C) to[out=135,in=0] (-1.5,1);
		\draw[-{Stealth[scale=1]}, red, thick, densely dashed] (C) to[out=-90,in=90] (0,-2.5);
		
	\end{tikzpicture}
\end{center}

\subsection*{Explanation of the Illustration}

In this illustration:
\begin{itemize}
	\item The large circle labeled \( C \) represents the complex plane as a function space.
	\item **Red Dashed Circle (Zero Ideal Localization)**: Surrounding the origin, representing the field of fractions \( C(x) \), which has rational functions accessible across the plane.
	\item **Red Circle at \( x = a \)**: Localization at the prime ideal \( (x - a) \), focusing on behavior around \( x = a \).
	\item **Blue Circle around \( f \)**: Localization at a function \( f \), where \( f \neq 0 \), indicating regions where \( f \) can be inverted.
\end{itemize}

Each arrow represents a "focus" that restricts attention to specific aspects of the function space.
\begin{tikzpicture}
	
	% Draw the polynomial plane C[x]
	\draw[fill=blue!10] (-3.5,-2.5) rectangle (3.5,2.5);
	\node at (-3.2,2.3) {\large $C[x]$};
	
	% Draw focus region for (x - a)
	\node at (2.3,1.5) {\small $(x - a)$};
	\draw[fill=red!20, opacity=0.4] (2,1) circle (0.5);
	\draw[fill=red!20, opacity=0.3] (2,1) circle (1);
	\draw[fill=red!20, opacity=0.2] (2,1) circle (1.5);
	\node[red] at (2,1) {\textbullet};
	\node[below right] at (2,0.9) {$x = a$};
	
	% Draw focus region for (0)
	\node at (-2.3,-1.5) {\small $(0)$};
	\draw[fill=green!20, opacity=0.4] (-2,-1) circle (0.5);
	\draw[fill=green!20, opacity=0.3] (-2,-1) circle (1);
	\draw[fill=green!20, opacity=0.2] (-2,-1) circle (1.5);
	\node[green!50!black] at (-2,-1) {\textbullet};
	\node[above left] at (-2,-1) {$x = 0$};
	
	% Add some labels to emphasize "localization"
	\node[above] at (2,1.8) {Localization near $x = a$};
	\node[below] at (-2,-1.8) {Localization near $x = 0$};
	
	% Optional arrows to show focusing effect
	\draw[->, thick, red] (2,2.2) -- (2,1.6);
	\draw[->, thick, green!50!black] (-2,-2.2) -- (-2,-1.6);
	
	% Boundary for the polynomial field plane
	\draw[blue!40, thick, dashed] (-3.5,-2.5) rectangle (3.5,2.5);
