...
\begin{center}
	\huge\textbf{Spectrum of a Ring and Structure Sheaf}\\
	\vspace{0.5em}
	\large{Ji, Yong-hyeon}\\
	\vspace{0.5em}
	\normalsize{\today}\\
...
\begin{center}
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
	{\mathbb{C}[x]} \arrow[dd, "\phi_\alpha"', shift right=2] &  &  &  & \{\langle 0\rangle\}\cup\{\langle x-\alpha\rangle\mid\alpha\in\mathbb{C}\} \\
	&  &  &  &                                                                         \\
	\mathbb{C} \arrow[uu, "f^{-1}"', shift right=2] &  &  &  & \{\langle 0\rangle\} \arrow[uu]                                        
\end{tikzcd}
\end{center}
\begin{itemize}
	\item Rings: $\C[x],\C$;
	\item The \textbf{evaluation homomorphism} at $x=\alpha$: \[
	\fullfunction{\phi_\alpha}{\C[x]}{\C}{p(x)}{p(\alpha)}
	\]
\end{itemize}

\section*{Spectrum of Ring}
\subsection*{$\mathbf{CRing}$: Category of Commutative Rings}
\begin{itemize}
	\item \textbf{Objects}: commutative rings $R,S,\dots$
	\item \textbf{Morphisms}: ring homomorphisms $f:R\to S, \dots$
\end{itemize}
\subsection*{$\mathbf{Top}$: The Category of Topological spaces}
\begin{itemize}
	\item \textbf{Objects}: topological spaces $(X,\tau_X), (Y,\tau_Y),\dots$
	\item \textbf{Morphisms}: continuous maps $f:(X,\tau_X)\to(Y,\tau_Y)$, which satisfy: $U\in\tau_Y\Rightarrow f^{-1}[U]\in\tau_X$.
\end{itemize}
\subsection*{The Functor $\mathbf{Spec}:\mathbf{CRing}^{\text{op}}\to\mathbf{Top}$}
\begin{itemize}
	\item \textbf{Objects}
	\begin{itemize}
		\item For a commutative ring $R$, $\text{Spec}(R)$ is the \textit{spectrum of} $R$: \[
		\text{Spec}(R)=\set{\mathfrak{p}\subseteq R:\mathfrak{p}\ \text{is a prime ideal}}.
		\]
		\item An ideal $\mathfrak{p}\ \rhd R$ is prime if \[
		ab\in\mathfrak{p}\implies a\in\mathfrak{p}\ \text{or}\ b\in\mathfrak{p}.
		\]
		\item \textit{Zariski Topology} on $\text{Spec}(R)$:
		
		The spectrum $\text{Spec}(R)$ is quipped with \textbf{Zariski topology}, where the closed sets are \[
		V(I)=\set{\mathfrak{p}\in\text{Spec}(R): I\subseteq\mathfrak{p}}
		\] for any ideal $I\rhd R$.
	\end{itemize}
	\item \textbf{Morphisms}
	
	For a ring homomorphism $f:R\to S$ in $\mathbf{CRing}$, the corresponding morphism in $\mathbf{Spec}$ is a continuous map \[
	\fullfunction{\mathbf{Spec}(f)}{\mathbf{Spec}(S)}{\mathbf{Spec}(R)}{\mathfrak{p}}{f^{-1}[\mathfrak{p}]},
	\] where $f^{-1}[\mathfrak{p}]=\set{r\in R: f(r)\in\mathfrak{p}}$ is the preimage of the prime ideal $\mathfrak{p}\in\mathbf{Spec}(S)$.
\end{itemize}
\vspace{40pt}
\begin{center}% https://tikzcd.yichuanshen.de/#-nSBkadBws1C0IcpS4QA
\begin{tikzcd}[remember picture]
	R \arrow[dd, "f"', shift right=2]      &                                        &  &    & \textbf{Spec}(R)                                \\
	& {} \arrow[rr, "\mathbf{Spec}", dashed] &  & {} &                                                 \\
	S \arrow[uu, "f^{-1}"', shift right=2] &                                        &  &    & \textbf{Spec}(S) \arrow[uu, "\textbf{Spec}(f)"]
\end{tikzcd}
\begin{tikzpicture}[overlay,remember picture]
\draw[dotted, red, line width=.5mm] (-8,-1.5) rectangle (-5.5,2) node[above] {Category $\mathbf{CRing}$};
\begin{scope}[xshift=5.5cm]
\draw[dotted, red, line width=.5mm] (-8,-1.5) rectangle (-5.5,2) node[above] {Category $\mathbf{Top}$};
\end{scope}
\end{tikzpicture}
\end{center}
